...
let MAX_CONCURRENT_REQUESTS = 10; 
 // Start with 100ms delay between requests
let REQUEST_DELAY = 100;
const MAX_RETRIES = 3;
...
// Providing Maximum Delay to prevent API overload
const MAX_DELAY = 1000; 

// One API request is approximately taking 170 ms of time.
const BASE_PROCESS_TIME = 170; 

const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

// Modify for more queries
const prefixes = generate(); 
...
const extractNames = async () => {
    ...
    for (let i = 0; i < prefixes.length; i += MAX_CONCURRENT_REQUESTS) {
        ...
        if (MAX_CONCURRENT_REQUESTS < 15) {
            // Slowly increase concurrency if stable
